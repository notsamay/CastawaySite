...
const DynamicText2 = ({ text1, text2, text3 }) => {
  const part1Ref = useRef(null);
  const part2Ref = useRef(null);
  const part3Ref = useRef(null);
  const [fontSize, setFontSize] = useState('2.75em'); // Default font size in em
  ...
  useEffect(() => {
    const adjustFontSize = () => {
      if (part1Ref.current && part2Ref.current && (!text3 || part3Ref.current)) {
        ...
        const newFontSizeEm = (part1Width / part2Width) * baseFontSize;
  ...
  return (
    <h2>
      <span className="cutout DTxt1" ref={part1Ref}>{text1}</span><br/>
      <span className="cutout DTxt2" ref={part2Ref} style={{ fontSize }}>{text2}</span>
